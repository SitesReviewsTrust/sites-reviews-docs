...
| **Let my AI assistant check sites** | The **[MCP server](https://github.com/SitesReviewsTrust/sites-reviews-mcp)** (`npx -y @sitesreviews/mcp`) |
...
Для разработчиков доступен **публичный REST API** (без ключа, только чтение): `https://sites.reviews/api/public/v1` — а для AI-ассистентов те же данные отдаёт **MCP-сервер** (`npx -y @sitesreviews/mcp`). Подробности — в [docs/api-and-tools.md](./docs/api-and-tools.md).
